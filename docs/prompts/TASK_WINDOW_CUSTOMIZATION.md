...
MuxueTools 桌面应用使用 `webview_go` 作为 WebView 封装层，当前窗口使用 Windows 原生标题栏。
...
```rc
// Windows Resource Script for MuxueTools Desktop
...
实现自定义样式的标题栏按钮（最小化、最大化、关闭），使其与 MuxueTools 的深色主题 UI 风格一致。
...
<template>
  <div v-if="isDesktopApp" class="title-bar">
    ...
    <div class="title-bar-left" @mousedown="startDrag">
      <img src="/icon.png" alt="MuxueTools" class="app-icon" />
      <span class="app-title">MuxueTools</span>
    </div>
