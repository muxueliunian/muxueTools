...
## 背景
MxlnAPI 目前运行在浏览器中。用户希望脱离默认浏览器，运行在独立的桌面窗口中，体验更像原生应用。
...
- Windows 构建命令示例：
  ```powershell
  # Windows x64
  go build -ldflags "-H windowsgui" -o bin/mxlnapi-win-amd64.exe ./cmd/desktop
  
  # Windows x86 (需 32位 gcc)
  $env:GOARCH="386"
  go build -ldflags "-H windowsgui" -o bin/mxlnapi-win-386.exe ./cmd/desktop
  ```
...
- `cmd/desktop/main.go`
- 更新后的构建脚本
- 可运行的 `mxlnapi-win-amd64.exe` (无控制台，独立窗口)
