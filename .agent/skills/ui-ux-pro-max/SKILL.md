---
name: UI/UX Pro Max
description: Design Intelligence - Searchable database of UI styles, color palettes, font pairings, chart types, product recommendations, UX guidelines, and stack-specific best practices. Use for any UI/UX design, build, create, implement, review, fix, or improve tasks.
---
...
**IMPORTANT:** Run the search script from the skill directory:

```bash
cd ~/.gemini/antigravity/skills/ui-ux-pro-max && python3 scripts/search.py "<keyword>" --domain <domain> [-n <max_results>]
```

**Or use absolute path on Windows:**
```powershell
python3 $env:USERPROFILE\.gemini\antigravity\skills\ui-ux-pro-max\scripts\search.py "<keyword>" --domain <domain> [-n <max_results>]
```
...
cd ~/.gemini/antigravity/skills/ui-ux-pro-max
