...
| 平台 | 支持状态 | 分发格式 |
|------|---------|---------|
| Windows x64 | ✅ 主要支持 | `mxlnapi-windows-amd64.exe` (WebView) |
| Windows x86 | ✅ 新增支持 | `mxlnapi-windows-386.exe` (WebView) |
| Linux x64 | ✅ 完全支持 | `mxlnapi-linux-amd64` |
| Linux ARM64 | ✅ 完全支持 | `mxlnapi-linux-arm64` |
| macOS x64 | ✅ 完全支持 | `mxlnapi-darwin-amd64` (WebView) |
| macOS ARM64 | ✅ 完全支持 | `mxlnapi-darwin-arm64` (WebView) |
| Android 15+ | ✅ 核心支持 | `mxlnapi.apk` (Native WebView) |
...
### Windows
```powershell
# 1. 下载 mxlnapi-windows-amd64.exe
...
```bash
# 1. 下载对应架构的二进制文件
wget https://github.com/muxueliunian/mxlnapi/releases/latest/download/mxlnapi-linux-amd64
chmod +x mxlnapi-linux-amd64
...
- [x] 项目名称：**MxlnAPI**
- [x] GitHub 仓库：`muxueliunian/mxlnapi`（待创建）
- [x] 无需用户认证，本地部署
